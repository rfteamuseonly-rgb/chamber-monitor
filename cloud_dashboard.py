...
with st.sidebar:
    ...
    st.markdown("---")
    st.info("💡 目前為「適中舒適」佈局：維持 4 欄位，略微縮減卡片留白與大小，讓 5F 與 6F 剛好能完美呈現在同一個畫面中。")

# ==========================================
# 1. 適中密度 CSS 樣式定義 (微調版)
# ==========================================
# 共通 CSS：稍微減少 Streamlit 預設的頂部留白，保留兩側舒適空間
common_css = """
<style>
...
# --- 風格 A: 經典簡約卡片 ---
# (稍微減少 padding 從 15px -> 12px，字體稍微縮減一點點)
css_classic = """
<style>
    ...
    .history-btn { background-color: #a9d0fc; color: white; padding: 2px 8px; border-radius: 4px; font-size: 0.8em; text-decoration: none; }
...
# --- 風格 B: 科技儀表板 (深色) ---
# (圓環從 85px 縮至 70px，padding 從 15px 縮至 12px)
css_modern = """
<style>
    ...
    .tech-header { display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #4b5070; padding-bottom: 6px; margin-bottom: 10px; }
    .tech-room { font-size: 1.25em; font-weight: bold; }
    .history-btn-tech { background-color: transparent; border: 1px solid #5c638c; color: #8fa1cd !important; padding: 2px 8px; border-radius: 12px; font-size: 0.75em; text-decoration: none; }
...
<style>
    ...
    .min-header { font-size: 1.1em; font-weight: bold; color: #222; margin-bottom: 12px; display: flex; justify-content: space-between;}
    ...
    if style_choice == "經典簡約卡片":
        ...
        <div class="tech-card status-{status}">
            <div class="tech-header">
                ...
                <a href="{sheet_link}" target="_blank" class="history-btn-tech">⛭ Hist</a>
            </div>
        ...
        <div class="neu-card neu-{status}">
            ...
            <div class="neu-data">溫度 <br><span class="neu-val">{temp_disp}</span></div>
