...
css_classic = "<style>.sensor-card { border-radius: 8px; padding: 12px; margin-bottom: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); border: 1px solid #e0e0e0; background-color: white; color: #333; } .status-green { border-left: 6px solid #28a745; background-color: #f5faf6; } .status-yellow { border-left: 6px solid #ffc107; background-color: #ffffeb; } .status-red { border-left: 6px solid #dc3545; background-color: #f8d7da; } .status-offline { border-left: 6px solid #6c757d; background-color: #f2f2f2; } .card-header { display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #eee; padding-bottom: 6px; margin-bottom: 8px;} .room-name { font-size: 1.25em; font-weight: bold; } .data-val { font-weight: bold; margin-left: 5px; font-size: 1.15em;} .data-row { margin: 3px 0; } .timestamp { font-size: 0.75em; color: #888; text-align: right; margin-top: 6px; }</style>"
...
<style>
    ...
    .cyber-card { background: #010103; border: 2px solid #0ff; border-radius: 8px; padding: 15px; margin-bottom: 15px; box-shadow: 0 0 10px rgba(0,255,255,0.5); position: relative; }
    .cyber-header { font-size: 1.4em; font-weight: bold; color: #0ff; text-shadow: 0 0 5px #0ff; margin-bottom: 10px; border-bottom: 1px dashed #0ff; padding-bottom: 5px; display: flex; justify-content: space-between;}
    .cyber-data { font-size: 1.1em; margin: 5px 0; color: #0ff; text-shadow: 0 0 5px #0ff; } 
    .cyber-val { color: #0f0; text-shadow: 0 0 8px #0f0; font-size: 1.2em; font-weight: bold;}
    .cyber-alert { border: 2px solid #f00; box-shadow: 0 0 15px rgba(255,0,0,0.8); animation: glitch-border 0.5s infinite; }
    ...
    .cyber-alert .cyber-data { color: #f00; text-shadow: 0 0 5px #f00; }
...
<style>
    ...
    .glass-card { background: rgba(255, 255, 255, 0.4); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); border: 1px solid rgba(255, 255, 255, 0.6); box-shadow: 0 8px 32px 0 rgba(31, 38, 135, 0.1); border-radius: 16px; padding: 15px; margin-bottom: 15px; position: relative; overflow: hidden;}
    ...
    .glass-data-block { background: rgba(52, 152, 219, 0.15); border-radius: 8px; padding: 8px 12px; text-align: center; }
    .glass-label { font-size: 0.8em; color: #555; margin-bottom: 4px; }
    .glass-val { font-size: 1.5em; font-weight: 800; color: #2c3e50; }
    .glass-liquid { position: absolute; bottom: -50%; left: -50%; width: 200%; height: 200%; background: rgba(52, 152, 219, 0.1); border-radius: 40%; animation: liquid-spin 6s linear infinite; z-index: 0; }
    ...
    .glass-alert .glass-data-block { background: rgba(231, 76, 60, 0.2); }
    .glass-alert .glass-liquid { background: rgba(231, 76, 60, 0.25); animation: liquid-spin 3s linear infinite; }
...
def render_card(chamber_id, data_dict):
    ...
    if style_choice == "經典簡約卡片":
        ...
        return f'<div class="cyber-card {alert_class}"><div class="cyber-header"><span>{chamber_id}</span><span class="cyber-warn-badge">SYS.WARN</span></div><div class="cyber-data">TMP: <span class="cyber-val">{temp_disp}</span></div><div class="cyber-data">HUM: <span class="cyber-val">{humi_disp}</span></div><div style="font-size:0.7em; color:#888; text-align:right; margin-top:8px;">LAST_SYNC: {time_disp}</div></div>'
    elif style_choice == "玻璃擬物 (液體波紋)":
        alert_class = "glass-alert" if status == "red" else ""
        return f'<div class="glass-card {alert_class}"><div class="glass-liquid" style="top: {100 - (humi_pct * 0.8)}%;"></div><div class="glass-header">{chamber_id}</div><div class="glass-data-row"><div class="glass-data-block"><div class="glass-label">溫度</div><div class="glass-val">{temp_disp}</div></div><div class="glass-data-block"><div class="glass-label">濕度</div><div class="glass-val">{humi_disp}</div></div></div></div>'
