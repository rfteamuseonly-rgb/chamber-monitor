...
with st.expander("⚙️ 點擊展開 / 隱藏介面設定 (風格切換)", expanded=False):
    style_choice = st.radio(
        ...
        index=4, # 預設切換到賽博龐克風格，以匹配 image_0.png
        horizontal=True
    )
...
<style>
    ...
    .cyber-card { 
        background: #010103; /* 深色卡片背景 */
        border: 2px solid #0ff; /* 藍綠色發光邊框 */
        border-radius: 8px; /* 模擬圖是圓角 */
        padding: 15px; 
        margin-bottom: 15px; 
        box-shadow: 0 0 10px rgba(0,255,255,0.5); /* 模擬圖發光效果 */
        position: relative; 
    }
    .cyber-header { 
        font-size: 1.4em; 
        font-weight: bold; 
        color: #0ff; 
        text-shadow: 0 0 5px #0ff; /* 房間號碼發光 */
        margin-bottom: 10px; 
        border-bottom: 1px dashed #0ff; /* 模擬圖分隔線 */
        padding-bottom: 5px; 
        display: flex; 
        justify-content: space-between;
    }
    .cyber-data { font-size: 1.1em; margin: 5px 0; color: #0ff; text-shadow: 0 0 5px #0ff; } /* 標籤發光 */
    .cyber-val { 
        color: #0f0; /* 鮮綠色數據 */
        text-shadow: 0 0 8px #0f0; /* 數據發光 */
        font-size: 1.2em; 
        font-weight: bold;
    }
    /* 警告狀態：紅色邊框、紅色數據、Glitch 效果 */
    ...
    # 🌟 修改 2：玻璃擬物 (匹配 image_1.png)
        ...
        <div class="glass-card {alert_class}">
            ...
            <div class="glass-data-row">
                <div class="glass-data-block">
                    <div class="glass-label">溫度</div>
                    <div class="glass-val">{temp_disp}</div>
                </div>
                <div class="glass-data-block">
                    <div class="glass-label">濕度</div>
                    <div class="glass-val">{humi_disp}</div>
                </div>
            </div>
        </div>
        """
        
    # 🌟 修改 3：極簡光環 (匹配 image_2.png)
            ...
            <div class="ring-container">
                <div style="text-align:center;">
                    <div class="ring-gauge" style="background: conic-gradient({color_t} {temp_pct}%, #edf2f7 0);">
                        <div class="ring-inner">Temp</div>
                    </div>
                    <div class="ring-data-val">{temp_disp}</div>
                </div>
                <div style="text-align:center;">
                    <div class="ring-gauge" style="background: conic-gradient({color_h} {humi_pct}%, #edf2f7 0);">
                        <div class="ring-inner">Humi</div>
                    </div>
                    <div class="ring-data-val">{humi_disp}</div>
                </div>
            </div>
        </div>
        """
