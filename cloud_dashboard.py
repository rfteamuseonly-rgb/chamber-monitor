...
with st.sidebar:
    ...
    st.markdown("---")
    st.info("💡 目前為「適中舒適」佈局，且已啟用瀏覽器層級自動重整 (每 5 分鐘)，介面操作更滑順。")
...
@st.cache_data(ttl=60) # 快取縮短為 60 秒，確保資料即時性
def get_latest_data():
    try:
...
def render_card(chamber_id, data_dict):
    ...
    if style_choice == "經典簡約卡片":
        return f'<div class="sensor-card status-{status}"><div class="card-header"><span class="room-name">{chamber_id}</span><a href="{sheet_link}" target="_blank" class="history-btn">History</a></div><div class="data-row">Temp: <span class="data-val">{temp_disp}</span></div><div class="data-row">Humidity: <span class="data-val">{humi_disp}</span></div></div>'
    elif style_choice == "科技儀表板 (深色)":
        return f'<div class="tech-card status-{status}"><div class="tech-header"><span class="tech-room">{chamber_id}</span><a href="{sheet_link}" target="_blank" class="history-btn-tech">⛭ Hist</a></div><div class="gauges-container"><div class="gauge-wrapper"><div class="gauge-ring"><span class="gauge-val">{temp_disp}</span></div><div class="gauge-title">Temp</div></div><div class="gauge-wrapper"><div class="gauge-ring"><span class="gauge-val">{humi_disp}</span></div><div class="gauge-title">Humi</div></div></div></div>'
        ...
        return f'<div class="min-card"><div class="min-header"><span>{chamber_id}</span> <span>{icon}</span></div><div class="min-label"><span>溫度</span> <span>{temp_disp}</span></div><div class="bar-bg"><div class="bar-fill-temp" style="width: {temp_pct}%;"></div></div><div class="min-label"><span>濕度</span> <span>{humi_disp}</span></div><div class="bar-bg"><div class="bar-fill-humi" style="width: {humi_pct}%;"></div></div></div>'

# ==========================================
# 4. 主畫面佈局 (移除 while True，改用 HTML 自動重整)
# ==========================================
# 🌟 關鍵修改：使用 HTML Meta 標籤讓「瀏覽器」幫我們每 300 秒自動 F5，不再卡死 Python！
